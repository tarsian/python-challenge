...
# Main coding
    ...
    sorted_list = sorted(voters_candi) 
...
print(f"{candi_vote[0][2][0]}: {set}% ({candi_vote[0][2][1]})")
print("-----------------------------------------------------")
...
with open(election_file, "w") as outfile:
    ...
    outfile.write("-----------------------------------------------------\n")
    outfile.write(f"{candi_vote[0][1][0]}: {dool}% ({candi_vote[0][0][1]})\n")
    outfile.write(f"{candi_vote[0][0][0]}: {hana}% ({candi_vote[0][1][1]})\n")
